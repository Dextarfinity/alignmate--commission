...
export default function Camera() {
  ...
  // --------------------- BLOCK A: smoothing & helpers ---------------------
  const lastKeypointsRef = useRef<any[] | null>(null); // for smoothing
  const SMOOTHING_ALPHA = 0.6; // 0 = no smoothing, 1 = locked to new

  const normalizeKeypointsToPixels = (
    keypoints: any[],
    canvas: HTMLCanvasElement
  ) => {
    if (!keypoints || !canvas) return [];
    const width = canvas.width;
    const height = canvas.height;
    const isNormalized = keypoints.every((kp) => kp && kp.x <= 1 && kp.y <= 1);

    return keypoints.map((kp) => {
      if (!kp) return { x: 0, y: 0, confidence: 0 };
      return {
        x: isNormalized ? kp.x * width : kp.x,
        y: isNormalized ? kp.y * height : kp.y,
        confidence: kp.confidence ?? kp.score ?? 0,
      };
    });
  };

  const smoothKeypoints = (prev: any[] | null, curr: any[]) => {
    if (!prev) return curr;
    return curr.map((kp, i) => {
      const p = prev[i] || { x: kp.x, y: kp.y, confidence: kp.confidence ?? 0 };
      return {
        x: p.x * SMOOTHING_ALPHA + kp.x * (1 - SMOOTHING_ALPHA),
        y: p.y * SMOOTHING_ALPHA + kp.y * (1 - SMOOTHING_ALPHA),
  ...
  const fetchWeeklyStats = async (showLoadingIndicator: boolean = false) => {
    try {
      ...
      if (showLoadingIndicator) {
        ...
        toast.success(
          `📊 Stats updated! ${totalScans} scans this week (${averageScore.toFixed(
            1
          )}% avg)`,
          {
            icon: "📈",
  ...
  const checkAndRunWeeklyAggregation = async (userId: string) => {
    try {
      ...
      if (!lastRun) {
        ...
        const lastRunWeekStart = new Date(
          lastRunDate.setDate(lastRunDate.getDate() - lastRunDate.getDay() + 1)
        );
  ...
  useEffect(() => {
    const initializeCamera = async () => {
      showLoading(
        ...
        "Setting up camera and loading data...",
  ...
  // --------------------- BLOCK B: Enhanced skeleton drawer ---------------------
  const drawSkeletonEnhanced = (rawKeypoints: any[]) => {
    if (!canvasRef.current || !videoRef.current) return;
    const canvas = canvasRef.current;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    ...
    const pixelKps = normalizeKeypointsToPixels(rawKeypoints, canvas);
    const smoothed = smoothKeypoints(lastKeypointsRef.current, pixelKps);
    lastKeypointsRef.current = smoothed;

    // Clear dynamic drawing
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    const drawGroup = (pairs: number[][], color: string) => {
      ...
      pairs.forEach(([a, b]) => {
        const A = smoothed[a],
          B = smoothed[b];
        if (!A || !B) return;
        if ((A.confidence ?? 0) > 0.25 && (B.confidence ?? 0) > 0.25) {
          ctx.beginPath();
          ctx.moveTo(A.x, A.y);
          ctx.lineTo(B.x, B.y);
          ctx.stroke();
    ...
    drawGroup(connections.leftArm, "#60A5FA");
    drawGroup(connections.rightArm, "#FBBF24");
    drawGroup(connections.torso, "#A78BFA");
    drawGroup(connections.leftLeg, "#EC4899");
    drawGroup(connections.rightLeg, "#34D399");

    smoothed.forEach((kp, idx) => {
      if (!kp || (kp.confidence ?? 0) <= 0.25) return;
      const radius = 6;
      let fill = "#ffffff";
      if (idx === 0) fill = "#FDE68A";
      else if (idx === 1) fill = "#F59E0B";
      else if (idx >= 2 && idx <= 4) fill = "#60A5FA";
      else if (idx >= 5 && idx <= 7) fill = "#FBBF24";
      else if (idx >= 8 && idx <= 10) fill = "#EC4899";
      else if (idx >= 11 && idx <= 14) fill = "#34D399";
    ...
    const get = (i: number) => smoothed[i] || { x: 0, y: 0, confidence: 0 };
    ...
    drawAngle(1, 2, 3, "LShoulder");
    drawAngle(1, 5, 6, "RShoulder");
    drawAngle(2, 3, 4, "LElbow");
    drawAngle(5, 6, 7, "RElbow");
    drawAngle(9, 10, 11, "LKnee");
    drawAngle(12, 13, 14, "RKnee");
  ...
  const runRealTimeDetection = async () => {
    ...
    try {
      ...
      if (result.keypoints) {
        drawSkeletonEnhanced(result.keypoints);
      }
      ...
      if (
        ...
        playSuccess();
        toast.success(
          `✅ ${currentPosture.toUpperCase()}: ${result.overall_score}% saved!`,
          {
            duration: 2000,
            icon: "💾",
          }
  ...
  const handleScan = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `✅ Hybrid analysis complete (source: ${hybridResult.source}):`,
          hybridResult
        );
        ...
        scanResult = {
          ...
          feedback:
            ...
            ` [${
              hybridResult.source === "local"
                ? "📱 Local AI"
                : hybridResult.source === "api"
                ? "☁️ Cloud API"
                : "🔄 Offline"
            }]`,
        ...
        const finalScore = Math.max(0, baseScore); // No artificial cap
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white flex flex-col">
      ...
      <div className="relative p-4 mt-auto flex-shrink-0 space-y-3">
        ...
        <div className="text-center text-xs text-slate-400">
          {isRealTimeActive ? (
            <span>
              ✨ Continuously analyzing posture • Auto-saves when score ≥75%
            </span>
          ) : (
            <span>
