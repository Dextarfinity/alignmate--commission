...
export const Camera = () => {
  ...
  // Function to fetch current week's statistics
  const fetchWeeklyStats = async () => {
    try {
      ...
      const { data: progressData, error } = await supabase
        ...
        .select('total_scans, successful_scans, average_score')
      ...
      if (progressData) {
        setWeeklyStats({
          totalScans: progressData.total_scans,
          successfulScans: progressData.successful_scans,
  ...
  // Function to manually update weekly progress for data consistency
  const updateWeeklyProgress = async (userId: string) => {
    try {
      ...
      if (progressError) {
        console.error('Error updating weekly progress:', progressError)
      } else {
  ...
  const handleScan = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        const { data: { session } } = await supabase.auth.getSession()
        if (session?.user) {
          // Insert into scan_history - this will automatically trigger weekly_progress update via database trigger
          ...
          if (scanError) {
            ...
            // Show user feedback that scan wasn't saved
            setScanResult({
              ...scanResult,
            ...
            // Explicitly verify and update weekly progress if needed
